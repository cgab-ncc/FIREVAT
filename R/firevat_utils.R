...
GetPCAWGMutSigs <- function()  {
    f <- "../data/reference/mutational_signatures/PCAWG_sigProfiler_SBS_Signatures_2018_03_28.csv"
...
GetCOSMICMutSigsEtiologiesColors <- function()  {
    f <- "../data/reference/mutational_signatures/COSMIC_Signatures_Groups_Colors.csv"
    df.cosmic.mut.sigs <- read.csv(f, 
                                   header = T,
                                   check.names = F,
                                   stringsAsFactors = F)
